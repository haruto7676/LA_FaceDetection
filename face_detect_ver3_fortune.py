...
if __name__ == '__main__':
    ...
    print(images)
    for image in images:
        image_dir=os.path.join("img/",image)
        cv2_images.append(cv2.imread(image_dir,cv2.IMREAD_UNCHANGED))


    
    # img = "img/fortune_0" + str(random.randint(1,7)) + ".png"
    ...
    while end_flag == True:
        ...
        if len(face_list) == 0:
            ...
            for (x, y, w, h) in face_list:
                ...
                # 重ね合わせたフレームを表示する
                c_frame = overlayImage(c_frame, cv2_images[1], (x, y-h), (w, h))
                cv2.imshow('Frame', c_frame)
